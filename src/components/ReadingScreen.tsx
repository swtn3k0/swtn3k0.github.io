    ...
    <div className="min-h-screen pt-20 pb-10 px-4 max-w-4xl mx-auto flex flex-col items-center">
      <AnimatePresence mode="wait">
        {step === 'question' && (
          ...
          >
            <h2 className="text-3xl font-serif mb-4 text-purple-900 dark:text-purple-200">Bạn đang nghĩ gì?</h2>
            <p className="text-purple-800 dark:text-purple-300/60 mb-8 text-sm italic">Hãy tập trung vào câu hỏi của bạn...</p>
            <form onSubmit={handleQuestionSubmit} className="relative">
              <textarea
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="Nhập câu hỏi của bạn tại đây..."
                className="w-full h-32 bg-purple-50 dark:bg-white/5 border border-purple-500/20 dark:border-white/10 rounded-2xl p-4 text-purple-900 dark:text-white placeholder-purple-400 focus:outline-none focus:border-purple-500/50 transition-colors resize-none"
                autoFocus
              />
              <button
                type="submit"
        ...
        {step === 'spread' && (
          ...
          >
            <button onClick={() => setStep('question')} className="flex items-center text-purple-700 dark:text-purple-300 mb-6 hover:text-purple-900 dark:hover:text-purple-100 transition-colors mx-auto">
              <ArrowLeft className="w-4 h-4 mr-2" /> Chỉnh sửa câu hỏi
            </button>
            <h2 className="text-3xl font-serif mb-8 text-purple-900 dark:text-purple-200">Chọn kiểu trải bài</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <button
                  ...
                  className="p-8 rounded-2xl bg-purple-50 dark:bg-white/5 border border-purple-500/20 dark:border-white/10 hover:bg-purple-100 dark:hover:bg-white/10 hover:border-purple-500/50 transition-all group text-left"
                >
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-2xl font-serif text-purple-700 dark:text-purple-300">{item.count}</span>
                    <Sparkles className="w-5 h-5 text-purple-600 dark:text-purple-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <h3 className="text-xl font-serif mb-2 text-purple-900 dark:text-white">{item.label}</h3>
                  <p className="text-sm text-purple-800 dark:text-purple-300/60">{item.desc}</p>
                </button>
              ))}
            </div>
          </motion.div>
        ...
        {step === 'shuffle' && (
          ...
          >
            <h2 className="text-3xl font-serif mb-4 text-purple-900 dark:text-purple-200">Các lá bài đang kết nối...</h2>
            <p className="text-purple-700/60 dark:text-purple-300/60 mb-12 italic">Hãy thả lỏng tâm trí và tập trung vào năng lượng của bạn.</p>
            <ShuffleDeck
        ...
        {step === 'result' && (
          ...
          >
            <div className="flex justify-between items-center mb-12">
              <h2 className="text-3xl font-serif text-purple-900 dark:text-purple-200">Kết quả trải bài</h2>
              <button onClick={reset} className="text-sm text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 uppercase tracking-widest font-bold">
                Trải bài mới
              </button>
            </div>
            ...
            >
              {drawnCards.map((drawn, i) => (
                ...
                >
                  {drawn.positionName && (
                    <span className="text-xs text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-4 font-bold">
                  ...
                  <div className="mt-6 text-center max-w-[200px]">
                    <h4 className="text-lg font-serif text-purple-900 dark:text-purple-200 mb-2">{drawn.card.name}</h4>
                    <p className="text-xs text-purple-700/70 dark:text-purple-300/70 italic">
            ...
            <div className="bg-purple-50 dark:bg-white/5 border border-purple-500/20 dark:border-white/10 rounded-3xl p-8 md:p-12 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-purple-500 to-transparent" />
              <div className="flex items-center mb-6">
                <Sparkles className="w-6 h-6 text-purple-600 dark:text-purple-400 mr-3" />
                <h3 className="text-2xl font-serif text-purple-900 dark:text-purple-200">Lời giải từ Vũ trụ</h3>
              </div>
              
              <div className="prose prose-purple dark:prose-invert max-w-none">
                {isInterpreting ? (
                  <div className={`space-y-4 ${settings.effectsEnabled ? 'animate-pulse' : ''}`}>
                    <div className="h-4 bg-purple-500/10 dark:bg-white/10 rounded w-3/4" />
                    <div className="h-4 bg-purple-500/10 dark:bg-white/10 rounded w-full" />
                    <div className="h-4 bg-purple-500/10 dark:bg-white/10 rounded w-5/6" />
                    <div className="h-4 bg-purple-500/10 dark:bg-white/10 rounded w-2/3" />
                  </div>
                ) : (
                  <div className="text-purple-900/90 dark:text-purple-100/90 leading-relaxed markdown-body">
